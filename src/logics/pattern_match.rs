pub fn pattern_match() {
    ...
    let Plant { flowering: t_or_f,mass: float_num, } = plant;
    
    assert_eq!(true, t_or_f);
    assert_eq!(10.0, float_num);
    ...
    let fish_chips = Meal::FishAndChips { 
        chip_proportion: 7.00 
    };

    if let Meal::FishAndChips { .. } = fish_chips {
        println!("I had a fish chips!");
    }
    ...
    match meal {
        ...
        Meal::FishAndChips { chip_proportion: _ } => {
            println!("I had fish and chips!");
        }
    ...
    // 'for' loop, just like 'let' accepts an irrefutable pattern
